  ...
  useEffect(() => {
    const handleSearchSessions = () => {
      ...
      const filtered = allSessions.filter((session) => {
        ...
        const matchesStack = selectedStack.length === 0 || selectedStack.includes(session.stack_name);
        const matchesLevel = selectedLevel.length === 0 || selectedLevel.includes(session.level_name);

        let matchesDate = true;
        if (selectedDate) {
          const normalizedSelectedDate = normalizeDate(selectedDate);
          matchesDate = sessionDate === normalizedSelectedDate;
        }

        return isFutureSession && matchesLanguage && matchesStack && matchesLevel && matchesDate;
      });

      setFilteredSessions(filtered);
    };

    handleSearchSessions(); // Chiamare la funzione ad ogni modifica di uno dei filtri
  }, [searchTerm, selectedStack, selectedLevel, selectedDate, allSessions]);
    ...
    <div className="user-home-page flex flex-col items-center w-full gap-5">
      ...
      {user && (
        <>
          ...
          <div className="w-full grid grid-col-1 grid:col-[1fr,2fr] gap-8">
            <div className="w-full lg:w-1/2">
              <SessionFilter
                searchTerm={searchTerm}
                setSearchTerm={setSearchTerm}
              ...
              />
              <div className="w-full flex justify-center my-4">
                <EventCalendar selectedDate={selectedDate} setSelectedDate={setSelectedDate} selectedTime={selectedTime} setSelectedTime={setSelectedTime} />
              </div>
              <div className="flex gap-4 mt-4 justify-center">
                <Button variant={"outline"} onClick={handleClearFilters}>
                  Limpiar filtros
