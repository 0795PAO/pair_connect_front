...
const LoginPage = () => {
  ...
  const handleSubmit = async (data) => {
    try {
      ...
      if (error.response) {
        ...
        toast({
          title: "Error",
          description: `Hubo un error inesperado, intentalo más tarde`,
          variant: "destructive",
        });
      }
    }
    ...
    try {
      ...
      if (response.status === 201) {
        toast({
          title: "Registrado",
          description:
            "Te has registrado correctamente! =D revisa tu correo para activar tu cuenta",
          variant: "success",
        });
  ...
  return (
    <>
      <h1
        className="mb-4 font-poppins font-bold text-5xl md:text-6xl leading-[120%] text-transparent bg-clip-text"
        style={{ backgroundImage: "var(--gradient)" }}
      >
        Pair Connect
      </h1>
      <div
        className="border rounded-lg md:w-[40vw] mouse-light-effect"
        ref={elementRef}
      >
