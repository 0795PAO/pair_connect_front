...
const SessionFilter = ({
    ...
    return (
        <div className="border rounded-lg mouse-light-effect h-fit" ref={elementRef}>
            <div className="p-5 card-with-light-effect" >
                <section className="w-full p-3">
                    <h4 className="text-xl font-semibold my-5">Filtra por lenguajes y Frameworks</h4>
                    <Input
                        type="text"
                    ...
                    <div className="mb-4 flex justify-evenly items-center">
                        {stacks && stacks.length > 0 && stacks.map((stack, index) => (
                            <CheckboxChip
                                key={index}
                                label={stack.label}
