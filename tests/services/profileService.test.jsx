...
describe('getUser', () => {
    it('should fetch and return user data successfully', async () => {
        ...
        expect(api.get).toHaveBeenCalledWith(PROFILE_URLS.GET); 
...
describe('updateUser', () => {
    it('should update and return the response successfully', async () => {
        const mockUpdateData = { name: 'John Doe Updated', email: 'john.updated@example.com' }; 
        ...
        expect(api.patch).toHaveBeenCalledWith(PROFILE_URLS.UPDATE, mockUpdateData); 
    ...
    it('should handle error when updating user data fails', async () => {
        ...
        await expect(updateUser({ name: 'Jane Doe' })).rejects.toThrow('Failed to update user data'); 
    });
});